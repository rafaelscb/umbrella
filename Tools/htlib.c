...
#undef pclose
#endif
#define stat(x, y) ht_stat((x), (y))
#define fopen(x, y) ht_fopen((x), (y))
#define open(x, y) ht_open((x), (y))
...
#define ENV_EXPORT_TO_CGI "UMSERVER_CGI"
#define PASSWORDS_FILE_NAME ".htpasswd"

#ifndef UMSERVER_USE_WEBSOCKET_PING_INTERVAL
#define UMSERVER_USE_WEBSOCKET_PING_INTERVAL 5
#endif

// Extra HTTP headers to send in every static file reply
#if !defined(UMSERVER_USE_EXTRA_HTTP_HEADERS)
#define UMSERVER_USE_EXTRA_HTTP_HEADERS ""
#endif

#ifndef UMSERVER_POST_SIZE_LIMIT
#define UMSERVER_POST_SIZE_LIMIT 0
#endif

#ifndef UMSERVER_IDLE_TIMEOUT_SECONDS
#define UMSERVER_IDLE_TIMEOUT_SECONDS 30
#endif

#ifdef UMSERVER_NO_SOCKETPAIR
#define UMSERVER_NO_CGI
#endif

#ifdef UMSERVER_NO_FILESYSTEM
#define UMSERVER_NO_AUTH
#define UMSERVER_NO_CGI
#define UMSERVER_NO_DAV
#define UMSERVER_NO_DIRECTORY_LISTING
#define UMSERVER_NO_LOGGING
#define UMSERVER_NO_SSI
#define UMSERVER_NO_DL
#endif
...
enum {
  ACCESS_CONTROL_LIST,
#ifndef UMSERVER_NO_FILESYSTEM
  ACCESS_LOG_FILE,
#ifndef UMSERVER_NO_AUTH
  AUTH_DOMAIN,
#endif
#ifndef UMSERVER_NO_CGI
  CGI_INTERPRETER,
  CGI_PATTERN,
#endif
  DAV_AUTH_FILE,
  DOCUMENT_ROOT,
#ifndef UMSERVER_NO_DIRECTORY_LISTING
  ENABLE_DIRECTORY_LISTING,
#endif
#endif
  EXTRA_MIME_TYPES,
#if !defined(UMSERVER_NO_FILESYSTEM) && !defined(UMSERVER_NO_AUTH)
  GLOBAL_AUTH_FILE,
#endif
#ifndef UMSERVER_NO_FILESYSTEM
  HIDE_FILES_PATTERN,
...
struct connection {
  ...
  struct ht_connection ht_conn;
  struct ht_server *server;
...
#define MG_CONN_2_CONN(c) ((struct connection *) ((char *) (c) - \
  offsetof(struct connection, ht_conn)))
...
#ifndef UMSERVER_NO_THREADS
void *ht_start_thread(void *(*f)(void *), void *p) {
  return ns_start_thread(f, p);
}
#endif // UMSERVER_NO_THREADS

#if defined(_WIN32) && !defined(UMSERVER_NO_FILESYSTEM)
...
static int ht_stat(const char *path, file_stat_t *st) {
...
static FILE *ht_fopen(const char *path, const char *mode) {
...
static int ht_open(const char *path, int flag) {
  wchar_t wpath[MAX_PATH_SIZE];
  to_wchar(path, wpath, ARRAY_SIZE(wpath));
  return _wopen(wpath, flag);
}
#endif // _WIN32 && !UMSERVER_NO_FILESYSTEM
...
static int ht_vsnprintf(char *buf, size_t buflen, const char *fmt, va_list ap) {
  int n;
...
static int ht_snprintf(char *buf, size_t buflen, const char *fmt, ...) {
  va_list ap;
  int n;
  va_start(ap, fmt);
  n = ht_vsnprintf(buf, buflen, fmt, ap);
  va_end(ap);
  return n;
}
...
static void parse_http_headers(char **buf, struct ht_connection *ri) {
  size_t i;
...
static int call_user(struct connection *conn, enum ht_event ev) {
  return conn != NULL && conn->server != NULL &&
    conn->server->event_handler != NULL ?
    conn->server->event_handler(&conn->ht_conn, ev) : MG_FALSE;
}
...
static void send_http_error(struct connection *conn, int code,
  ...
  while ((rewrites = next_option(rewrites, &a, &b)) != NULL) {
    if ((match_code = atoi(a.ptr)) > 0 && match_code == code) {
      struct ht_connection *c = &conn->ht_conn;
      c->status_code = 302;
      ht_printf(c, "HTTP/1.1 %d Moved\r\n"
  ...
  body_len = ht_snprintf(body, sizeof(body), "%d %s\n", code, message);
  if (fmt != NULL) {
    va_start(ap, fmt);
    body_len += ht_vsnprintf(body + body_len, sizeof(body) - body_len, fmt, ap);
    va_end(ap);
  }
  ...
  headers_len = ht_snprintf(headers, sizeof(headers),
                            "HTTP/1.1 %d %s\r\nContent-Length: %d\r\n"
...
static void write_chunk(struct connection *conn, const char *buf, int len) {
  char chunk_size[50];
  int n = ht_snprintf(chunk_size, sizeof(chunk_size), "%X\r\n", len);
...
static process_id_t start_process(char *interp, const char *cmd,
  ...
  ht_snprintf(cmdline, sizeof(cmdline), "%s%s\"%s\"",
...
static char *addenv(struct cgi_env_block *block, const char *fmt, ...) {
  ...
  va_start(ap, fmt);
  n = ht_vsnprintf(added, (size_t) space, fmt, ap);
  va_end(ap);
...
static void prepare_cgi_environment(struct connection *conn,
  ...
  struct ht_connection *ri = &conn->ht_conn;
  const char *s, *slash;
  ...
  addenv(blk, "SERVER_SOFTWARE=%s/%s", "UMSERVER", UMSERVER_VERSION);
  ...
  if ((s = ht_get_header(ri, "Content-Type")) != NULL)
    addenv(blk, "CONTENT_TYPE=%s", s);

  if (ri->query_string != NULL)
    addenv(blk, "QUERY_STRING=%s", ri->query_string);

  if ((s = ht_get_header(ri, "Content-Length")) != NULL)
...
static void open_cgi_endpoint(struct connection *conn, const char *prog) {
  ...
  if ((p = strrchr(prog, '/')) == NULL) {
    ht_snprintf(dir, sizeof(dir), "%s", ".");
  } else {
    ht_snprintf(dir, sizeof(dir), "%.*s", (int) (p - prog), prog);
  }
...
static void on_cgi_data(struct ns_connection *nc) {
  ...
  if (conn->ns_conn->flags & NSF_BUFFER_BUT_DONT_SEND) {
    ...
    if (len < 0 || len > (int) sizeof(buf)) {
      ...
      if (ht_get_header(&c, "Location") != NULL) {
        status = "302";
      } else if ((status = (char *) ht_get_header(&c, "Status")) == NULL) {
        status = "200";
      }
      memcpy(io->buf + 9, status, 3);
      conn->ht_conn.status_code = atoi(status);
    }
    conn->ns_conn->flags &= ~NSF_BUFFER_BUT_DONT_SEND;
  }
}
#endif // !UMSERVER_NO_CGI
...
int ht_url_decode(const char *src, int src_len, char *dst,
...
// Parse HTTP request, fill in ht_request structure.
// This function modifies the buffer by NUL-terminating
// HTTP request components, header names and header values.
// Note that len must point to the last \n of HTTP headers.
static int parse_http_message(char *buf, int len, struct ht_connection *ri) {
  int is_request, n;
  ...
  if ((is_request && memcmp(ri->http_version, "HTTP/", 5) != 0) ||
    ...
    n = (int) strlen(ri->uri);
    ht_url_decode(ri->uri, n, (char *) ri->uri, n + 1, 0);
    if (*ri->uri == '/' || *ri->uri == '.') {
...
static int ht_strcasecmp(const char *s1, const char *s2) {
  int diff;
...
static int ht_strncasecmp(const char *s1, const char *s2, size_t len) {
  int diff = 0;
...
const char *ht_get_header(const struct ht_connection *ri, const char *s) {
  int i;

  for (i = 0; i < ri->num_headers; i++)
    if (!ht_strcasecmp(s, ri->http_headers[i].name))
...
int ht_match_prefix(const char *pattern, int pattern_len, const char *str) {
  const char *or_str;
  int len, res, i = 0, j = 0;

  if ((or_str = (const char *) memchr(pattern, '|', pattern_len)) != NULL) {
    res = ht_match_prefix(pattern, or_str - pattern, str);
    return res > 0 ? res : ht_match_prefix(or_str + 1,
  ...
  for (; i < pattern_len; i++, j++) {
    if (pattern[i] == '?' && str[j] != '\0') {
      ...
      do {
        res = ht_match_prefix(pattern + i, pattern_len - i, str + j + len);
      } while (res == -1 && len-- > 0);
...
void ht_template(struct ht_connection *conn, const char *s,
...
static int must_hide_file(struct connection *conn, const char *path) {
  ...
  return ht_match_prefix(pw_pattern, strlen(pw_pattern), path) > 0 ||
    (pattern != NULL && ht_match_prefix(pattern, strlen(pattern), path) > 0);
}
...
static int convert_uri_to_file_name(struct connection *conn, char *buf,
  ...
  char *p;
#endif
  const char *uri = conn->ht_conn.uri;
  const char *domain = ht_get_header(&conn->ht_conn, "Host");
  ...
  if (rewrites != NULL && domain != NULL) {
    ...
    while ((rewrites = next_option(rewrites, &a, &b)) != NULL) {
      if (a.len > 1 && a.ptr[0] == '@' && a.len == domain_len + 1 &&
          ht_strncasecmp(a.ptr + 1, domain, domain_len) == 0) {
        root = b.ptr;
        root_len = b.len;
        break;
      }
  ...
  // Handle URL rewrites
  ht_snprintf(buf, buf_len, "%.*s%s", root_len, root, uri);
  rewrites = conn->server->config_options[URL_REWRITES];  // Re-initialize!
  while ((rewrites = next_option(rewrites, &a, &b)) != NULL) {
    if ((match_len = ht_match_prefix(a.ptr, a.len, uri)) > 0) {
      ht_snprintf(buf, buf_len, "%.*s%s", (int) b.len, b.ptr, uri + match_len);
      break;
    }
  }
  ...
  for (p = buf + strlen(root) + 2; *p != '\0'; p++) {
    if (*p == '/') {
      *p = '\0';
      if (ht_match_prefix(cgi_pat, strlen(cgi_pat), buf) > 0 &&
          !stat(buf, st)) {
      DBG(("!!!! [%s]", buf));
        *p = '/';
        conn->path_info = ht_strdup(p);
        *p = '\0';
        return 1;
      }
      *p = '/';
...
#endif  // UMSERVER_NO_FILESYSTEM

static int should_keep_alive(const struct ht_connection *conn) {
  struct connection *c = MG_CONN_2_CONN(conn);
  const char *method = conn->request_method;
  const char *http_version = conn->http_version;
  const char *header = ht_get_header(conn, "Connection");
  return method != NULL &&
    (!strcmp(method, "GET") || c->endpoint_type == EP_USER) &&
    ((header != NULL && !ht_strcasecmp(header, "keep-alive")) ||
     (header == NULL && http_version && !strcmp(http_version, "1.1")));
}

int ht_write(struct ht_connection *c, const void *buf, int len) {
  struct connection *conn = MG_CONN_2_CONN(c);
  return ns_send(conn->ns_conn, buf, len);
}

void ht_send_status(struct ht_connection *c, int status) {
  if (c->status_code == 0) {
    c->status_code = status;
    ht_printf(c, "HTTP/1.1 %d %s\r\n", status, status_code_to_str(status));
  }
}

void ht_send_header(struct ht_connection *c, const char *name, const char *v) {
  if (c->status_code == 0) {
    c->status_code = 200;
...
static void terminate_headers(struct ht_connection *c) {
  struct connection *conn = MG_CONN_2_CONN(c);
  if (!(conn->ns_conn->flags & MG_HEADERS_SENT)) {
    ht_send_header(c, "Transfer-Encoding", "chunked");
    ht_write(c, "\r\n", 2);
    conn->ns_conn->flags |= MG_HEADERS_SENT;
  }
}

void ht_send_data(struct ht_connection *c, const void *data, int data_len) {
  terminate_headers(c);
  write_chunk(MG_CONN_2_CONN(c), (const char *) data, data_len);
}

void ht_printf_data(struct ht_connection *c, const char *fmt, ...) {
  struct connection *conn = MG_CONN_2_CONN(c);
  va_list ap;
  int len;
...
#if !defined(UMSERVER_NO_WEBSOCKET) || !defined(UMSERVER_NO_AUTH)
...
static void send_websocket_handshake(struct ht_connection *conn,
                                     const char *key) {
  static const char *magic = "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
  char buf[500], sha[20], b64_sha[sizeof(sha) * 2];
  SHA1_CTX sha_ctx;

  ht_snprintf(buf, sizeof(buf), "%s%s", key, magic);
  SHA1Init(&sha_ctx);
  SHA1Update(&sha_ctx, (unsigned char *) buf, strlen(buf));
  SHA1Final((unsigned char *) sha, &sha_ctx);
  base64_encode((unsigned char *) sha, sizeof(sha), b64_sha);
  ht_snprintf(buf, sizeof(buf), "%s%s%s",
              "HTTP/1.1 101 Switching Protocols\r\n"
              "Upgrade: websocket\r\n"
              "Connection: Upgrade\r\n"
              "Sec-WebSocket-Accept: ", b64_sha, "\r\n\r\n");

  ht_write(conn, buf, strlen(buf));
}
...
static int deliver_websocket_frame(struct connection *conn) {
  ...
  if (buffered) {
    conn->ht_conn.content_len = data_len;
    conn->ht_conn.content = (char *) buf + header_len;
    conn->ht_conn.wsbits = buf[0];
...
int ht_websocket_write(struct ht_connection* conn, int opcode,
    ...
    if (copy_len > 0) {
      retval = ht_write(conn, copy, copy_len);
    }
    if (copy != mem) {
      free(copy);
    }
...
int ht_websocket_printf(struct ht_connection* conn, int opcode,
  ...
  va_start(ap, fmt);
  if ((len = ns_avprintf(&buf, sizeof(mem), fmt, ap)) > 0) {
    ht_websocket_write(conn, opcode, buf, len);
  }
  va_end(ap);
...
static void send_websocket_handshake_if_requested(struct ht_connection *conn) {
  const char *ver = ht_get_header(conn, "Sec-WebSocket-Version"),
        *key = ht_get_header(conn, "Sec-WebSocket-Key");
...
static void ping_idle_websocket_connection(struct connection *conn, time_t t) {
  if (t - conn->ns_conn->last_io_time > UMSERVER_USE_WEBSOCKET_PING_INTERVAL) {
    ht_websocket_write(&conn->ht_conn, WEBSOCKET_OPCODE_PING, "", 0);
  }
}
#else
#define ping_idle_websocket_connection(conn, t)
#endif // !UMSERVER_NO_WEBSOCKET

static void write_terminating_chunk(struct connection *conn) {
  ht_write(&conn->ht_conn, "0\r\n\r\n", 5);
}

static int call_request_handler(struct connection *conn) {
  int result;
  conn->ht_conn.content = conn->ns_conn->recv_iobuf.buf;
...
const char *ht_get_mime_type(const char *path, const char *default_mime_type) {
  const char *ext;
  size_t i, path_len;
  ...
  for (i = 0; static_builtin_mime_types[i].extension != NULL; i++) {
    ...
    if (path_len > static_builtin_mime_types[i].ext_len &&
        ht_strcasecmp(ext, static_builtin_mime_types[i].extension) == 0) {
...
static void get_mime_type(const struct ht_server *server, const char *path,
                          struct vec *vec) {
  ...
  while ((list = next_option(list, &ext_vec, &mime_vec)) != NULL) {
    ...
    if (ht_strncasecmp(ext, ext_vec.ptr, ext_vec.len) == 0) {
      *vec = mime_vec;
      return;
    }
  }

  vec->ptr = ht_get_mime_type(path, "text/plain");
  vec->len = strlen(vec->ptr);
}

static const char *suggest_connection_header(const struct ht_connection *conn) {
  return should_keep_alive(conn) ? "keep-alive" : "close";
}

static void construct_etag(char *buf, size_t buf_len, const file_stat_t *st) {
  ht_snprintf(buf, buf_len, "\"%lx.%" INT64_FMT "\"",
...
static int is_not_modified(const struct connection *conn,
  ...
  char etag[64];
  const char *ims = ht_get_header(&conn->ht_conn, "If-Modified-Since");
  const char *inm = ht_get_header(&conn->ht_conn, "If-None-Match");
  construct_etag(etag, sizeof(etag), stp);
  return (inm != NULL && !ht_strcasecmp(etag, inm)) ||
...
static void open_file_endpoint(struct connection *conn, const char *path,
  ...
  r1 = r2 = 0;
  hdr = ht_get_header(&conn->ht_conn, "Range");
  if (hdr != NULL && (n = parse_range_header(hdr, &r1, &r2)) > 0 &&
      r1 >= 0 && r2 >= 0) {
    conn->ht_conn.status_code = 206;
    conn->cl = n == 2 ? (r2 > conn->cl ? conn->cl : r2) - r1 + 1: conn->cl - r1;
    ht_snprintf(range, sizeof(range), "Content-Range: bytes "
  ...
  n = ht_snprintf(headers, sizeof(headers),
                  "HTTP/1.1 %d %s\r\n"
                  "Date: %s\r\n"
                  "Last-Modified: %s\r\n"
                  "Etag: %s\r\n"
                  ...
                  "Accept-Ranges: bytes\r\n"
                  "%s%s\r\n",
                  conn->ht_conn.status_code, msg, date, lm, etag,
                  (int) mime_vec.len, mime_vec.ptr, conn->cl,
                  suggest_connection_header(&conn->ht_conn),
                  range, UMSERVER_USE_EXTRA_HTTP_HEADERS);
  ns_send(conn->ns_conn, headers, n);

  if (!strcmp(conn->ht_conn.request_method, "HEAD")) {
    conn->ns_conn->flags |= NSF_FINISHED_SENDING_DATA;
...
#if !defined(UMSERVER_NO_DIRECTORY_LISTING) || !defined(UMSERVER_NO_DAV)
...
static int scan_directory(struct connection *conn, const char *dir,
  ...
  *arr = NULL;
  ...
  while ((dp = readdir(dirp)) != NULL) {
    ...
    ht_snprintf(path, sizeof(path), "%s%c%s", dir, '/', dp->d_name);
...
#endif  // !NO_DIRECTORY_LISTING || !UMSERVER_NO_DAV
...
static void print_dir_entry(const struct dir_entry *de) {
  ...
  if (is_dir) {
    ht_snprintf(size, sizeof(size), "%s", "[DIRECTORY]");
  } else {
     // We use (signed) cast below because MSVC 6 compiler cannot
     // convert unsigned __int64 to double.
    if (fsize < 1024) {
      ht_snprintf(size, sizeof(size), "%d", (int) fsize);
    } else if (fsize < 0x100000) {
      ht_snprintf(size, sizeof(size), "%.1fk", (double) fsize / 1024.0);
    } else if (fsize < 0x40000000) {
      ht_snprintf(size, sizeof(size), "%.1fM", (double) fsize / 1048576);
    } else {
      ht_snprintf(size, sizeof(size), "%.1fG", (double) fsize / 1073741824);
    }
  }
  strftime(mod, sizeof(mod), "%d-%b-%Y %H:%M", localtime(&de->st.st_mtime));
  ht_url_encode(de->file_name, strlen(de->file_name), href, sizeof(href));
  ht_printf_data(&de->conn->ht_conn,
...
static int __cdecl compare_dir_entries(const void *p1, const void *p2) {
  ...
  const char *qs = a->conn->ht_conn.query_string ?
    a->conn->ht_conn.query_string : "na";
  int cmp_result = 0;
...
static void send_directory_listing(struct connection *conn, const char *dir) {
  ...
  int i, num_entries, sort_direction = conn->ht_conn.query_string != NULL &&
    conn->ht_conn.query_string[1] == 'd' ? 'a' : 'd';

  ht_send_header(&conn->ht_conn, "Transfer-Encoding", "chunked");
  ht_send_header(&conn->ht_conn, "Content-Type", "text/html; charset=utf-8");

  ht_printf_data(&conn->ht_conn,
              ...
              conn->ht_conn.uri, conn->ht_conn.uri,
...
static void handle_propfind(struct connection *conn, const char *path,
  ...
  const char *depth = ht_get_header(&conn->ht_conn, "Depth"),
        *list_dir = conn->server->config_options[ENABLE_DIRECTORY_LISTING];

  conn->ht_conn.status_code = 207;

  // Print properties for the requested resource itself
  if (!exists) {
    conn->ht_conn.status_code = 404;
    ht_printf(&conn->ht_conn, "%s", "HTTP/1.1 404 Not Found\r\n\r\n");
  } else if (S_ISDIR(stp->st_mode) && ht_strcasecmp(list_dir, "yes") != 0) {
    conn->ht_conn.status_code = 403;
    ht_printf(&conn->ht_conn, "%s",
              "HTTP/1.1 403 Directory Listing Denied\r\n\r\n");
  } else {
    ns_send(conn->ns_conn, header, sizeof(header) - 1);
    print_props(conn, conn->ht_conn.uri, stp);
    ...
    if (S_ISDIR(stp->st_mode) &&
      ...
      for (i = 0; i < num_entries; i++) {
        ...
        ht_url_encode(de->file_name, strlen(de->file_name), buf, sizeof(buf));
...
static void handle_put(struct connection *conn, const char *path) {
  file_stat_t st;
  const char *range, *cl_hdr = ht_get_header(&conn->ht_conn, "Content-Length");
  int64_t r1, r2;
  int rc;

  conn->ht_conn.status_code = !stat(path, &st) ? 200 : 201;
  if ((rc = put_dir(path)) == 0) {
    ht_printf(&conn->ht_conn, "HTTP/1.1 %d OK\r\n\r\n",
              conn->ht_conn.status_code);
    close_local_endpoint(conn);
  } else if (rc == -1) {
    ...
    ns_set_close_on_exec(conn->endpoint.fd);
    range = ht_get_header(&conn->ht_conn, "Content-Range");
    conn->cl = to64(cl_hdr);
    r1 = r2 = 0;
...
#ifndef UMSERVER_NO_AUTH
void ht_send_digest_auth_request(struct ht_connection *c) {
  struct connection *conn = MG_CONN_2_CONN(c);
  c->status_code = 401;
  ht_printf(c,
...
static FILE *open_auth_file(struct connection *conn, const char *path) {
  ...
  if (gpass != NULL) {
    ...
    ht_snprintf(name, sizeof(name), "%s%c%s", path, '/', PASSWORDS_FILE_NAME);
    fp = fopen(name, "r");
  } else {
    // Try to find .htpasswd in requested directory.
    if ((p = strrchr(path, '/')) == NULL) p = path;
    ht_snprintf(name, sizeof(name), "%.*s%c%s",
...
#if !defined(HAVE_MD5) && !defined(UMSERVER_NO_AUTH)
...
static int check_password(const char *method, const char *ha1, const char *uri,
  ...
  ht_md5(ha2, method, ":", uri, NULL);
  ht_md5(expected_response, ha1, ":", nonce, ":", nc,
      ":", cnonce, ":", qop, ":", ha2, NULL);

  return ht_strcasecmp(response, expected_response) == 0 ? MG_TRUE : MG_FALSE;
}
...
int ht_authorize_digest(struct ht_connection *c, FILE *fp) {
  ...
  if ((hdr = ht_get_header(c, "Authorization")) == NULL ||
      ht_strncasecmp(hdr, "Digest ", 7) != 0) return 0;
  if (!ht_parse_header(hdr, "username", user, sizeof(user))) return 0;
  if (!ht_parse_header(hdr, "cnonce", cnonce, sizeof(cnonce))) return 0;
  if (!ht_parse_header(hdr, "response", resp, sizeof(resp))) return 0;
  if (!ht_parse_header(hdr, "uri", uri, sizeof(uri))) return 0;
  if (!ht_parse_header(hdr, "qop", qop, sizeof(qop))) return 0;
  if (!ht_parse_header(hdr, "nc", nc, sizeof(nc))) return 0;
  if (!ht_parse_header(hdr, "nonce", nonce, sizeof(nonce))) return 0;
...
static int is_authorized(struct connection *conn, const char *path) {
  ...
  if ((fp = open_auth_file(conn, path)) != NULL) {
    authorized = ht_authorize_digest(&conn->ht_conn, fp);
    fclose(fp);
  }
...
static int is_authorized_for_dav(struct connection *conn) {
  ...
  const char *method = conn->ht_conn.request_method;
  FILE *fp;
...
int ht_parse_header(const char *s, const char *var_name, char *buf,
                    size_t buf_size) {
  return parse_header(s, s == NULL ? 0 : strlen(s), var_name, buf, buf_size);
}

#ifndef UMSERVER_NO_SSI
static void send_ssi_file(struct ht_connection *, const char *, FILE *, int);

static void send_file_data(struct ht_connection *conn, FILE *fp) {
  char buf[IOBUF_SIZE];
  int n;
...
static void do_ssi_include(struct ht_connection *conn, const char *ssi,
  ...
  if (sscanf(tag, " virtual=\"%[^\"]\"", file_name) == 1) {
    ...
    ht_snprintf(path, sizeof(path), "%s%c%s",
                opts[DOCUMENT_ROOT], '/', file_name);
  } else if (sscanf(tag, " abspath=\"%[^\"]\"", file_name) == 1) {
    // File name is relative to the webserver working directory
    // or it is absolute system path
    ht_snprintf(path, sizeof(path), "%s", file_name);
  } else if (sscanf(tag, " file=\"%[^\"]\"", file_name) == 1 ||
             sscanf(tag, " \"%[^\"]\"", file_name) == 1) {
    // File name is relative to the currect document
    ht_snprintf(path, sizeof(path), "%s", ssi);
    if ((p = strrchr(path, '/')) != NULL) {
      p[1] = '\0';
    }
    ht_snprintf(path + strlen(path), sizeof(path) - strlen(path), "%s",
                file_name);
  } else {
    ht_printf(conn, "Bad SSI #include: [%s]", tag);
    return;
  }

  if ((fp = fopen(path, "rb")) == NULL) {
    ht_printf(conn, "Cannot open SSI #include: [%s]: fopen(%s): %s",
              tag, path, strerror(errno));
  } else {
    ns_set_close_on_exec(fileno(fp));
    if (ht_match_prefix(opts[SSI_PATTERN], strlen(opts[SSI_PATTERN]),
        path) > 0) {
...
#ifndef UMSERVER_NO_POPEN
static void do_ssi_exec(struct ht_connection *conn, char *tag) {
  char cmd[IOBUF_SIZE];
  FILE *fp;

  if (sscanf(tag, " \"%[^\"]\"", cmd) != 1) {
    ht_printf(conn, "Bad SSI #exec: [%s]", tag);
  } else if ((fp = popen(cmd, "r")) == NULL) {
    ht_printf(conn, "Cannot SSI #exec: [%s]: %s", cmd, strerror(errno));
  } else {
    send_file_data(conn, fp);
    pclose(fp);
  }
}
#endif // !UMSERVER_NO_POPEN

static void send_ssi_file(struct ht_connection *conn, const char *path,
                          FILE *fp, int include_level) {
  char buf[IOBUF_SIZE];
  int ch, offset, len, in_ssi_tag;

  if (include_level > 10) {
    ht_printf(conn, "SSI #include level is too deep (%s)", path);
    return;
  }
  ...
  while ((ch = fgetc(fp)) != EOF) {
    if (in_ssi_tag && ch == '>') {
      ...
      if (len < 6 || memcmp(buf, "<!--#", 5) != 0) {
        ...
        (void) ht_write(conn, buf, (size_t) len);
      } else {
        if (!memcmp(buf + 5, "include", 7)) {
          do_ssi_include(conn, path, buf + 12, include_level);
#if !defined(UMSERVER_NO_POPEN)
        } else if (!memcmp(buf + 5, "exec", 4)) {
          do_ssi_exec(conn, buf + 9);
#endif // !NO_POPEN
        } else {
          ht_printf(conn, "%s: unknown SSI " "command: \"%s\"", path, buf);
        }
      }
      len = 0;
    } else if (in_ssi_tag) {
      if (len == 5 && memcmp(buf, "<!--#", 5) != 0) {
        ...
        ht_printf(conn, "%s: SSI tag is too large", path);
        len = 0;
      }
      buf[len++] = ch & 0xff;
    } else if (ch == '<') {
...
static void handle_ssi_request(struct connection *conn, const char *path) {
  ...
  if ((fp = fopen(path, "rb")) == NULL) {
    ...
    ht_printf(&conn->ht_conn,
              ...
              "Content-Type: %.*s\r\n"
              "Connection: close\r\n\r\n",
              conn->ht_conn.status_code, (int) mime_vec.len, mime_vec.ptr);
    send_ssi_file(&conn->ht_conn, path, fp, 0);
    fclose(fp);
    close_local_endpoint(conn);
  }
}
...
static void proxy_request(struct ns_connection *pc, struct ht_connection *c) {
  int i, sent_close_header = 0;

  ns_printf(pc, "%s %s HTTP/%s\r\n", c->request_method, c->uri,
            c->http_version);
  for (i = 0; i < c->num_headers; i++) {
    if (ht_strcasecmp(c->http_headers[i].name, "Connection") == 0) {
...
static void proxify_connection(struct connection *conn) {
  ...
  // Find out whether we should be in the MITM mode
  {
    ...
    while ((certs = next_option(certs, &a, &b)) != NULL) {
      if (a.len == host_len && ht_strncasecmp(a.ptr, host, a.len) == 0) {
...
static void open_local_endpoint(struct connection *conn, int skip_user) {
  ...
  if (conn->server->event_handler && call_user(conn, MG_AUTH) == MG_FALSE) {
    ht_send_digest_auth_request(&conn->ht_conn);
    return;
  }
#endif
...
#if UMSERVER_POST_SIZE_LIMIT > 1
    {
      const char *cl = ht_get_header(&conn->ht_conn, "Content-Length");
      if ((strcmp(conn->ht_conn.request_method, "POST") == 0 ||
           strcmp(conn->ht_conn.request_method, "PUT") == 0) &&
          (cl == NULL || to64(cl) > UMSERVER_POST_SIZE_LIMIT)) {
        send_http_error(conn, 500, "POST size > %zu",
                        (size_t) UMSERVER_POST_SIZE_LIMIT);
      }
    }
#endif
    return;
  ...
  if (strcmp(conn->ht_conn.request_method, "CONNECT") == 0 ||
      memcmp(conn->ht_conn.uri, "http", 4) == 0) {
    proxify_connection(conn);
    return;
  }

#ifdef UMSERVER_NO_FILESYSTEM
  if (!strcmp(conn->ht_conn.request_method, "OPTIONS")) {
    send_options(conn);
  } else {
    send_http_error(conn, 404, NULL);
  }
...
#endif
#ifndef UMSERVER_NO_DAV
  } else if (!strcmp(conn->ht_conn.request_method, "PROPFIND")) {
    handle_propfind(conn, path, &st, exists);
  } else if (!strcmp(conn->ht_conn.request_method, "MKCOL")) {
    handle_mkcol(conn, path);
  } else if (!strcmp(conn->ht_conn.request_method, "DELETE")) {
    handle_delete(conn, path);
  } else if (!strcmp(conn->ht_conn.request_method, "PUT")) {
    handle_put(conn, path);
#endif
  } else if (!exists || must_hide_file(conn, path)) {
    send_http_error(conn, 404, NULL);
  } else if (is_directory &&
             conn->ht_conn.uri[strlen(conn->ht_conn.uri) - 1] != '/') {
    conn->ht_conn.status_code = 301;
    ht_printf(&conn->ht_conn, "HTTP/1.1 301 Moved Permanently\r\n"
              "Location: %s/\r\n\r\n", conn->ht_conn.uri);
    close_local_endpoint(conn);
  } else if (is_directory && !find_index_file(conn, path, sizeof(path), &st)) {
    if (!ht_strcasecmp(dir_lst, "yes")) {
#ifndef UMSERVER_NO_DIRECTORY_LISTING
  ...
  } else if (ht_match_prefix(cgi_pat, strlen(cgi_pat), path) > 0) {
#if !defined(UMSERVER_NO_CGI)
    open_cgi_endpoint(conn, path);
#else
    send_http_error(conn, 501, NULL);
#endif // !UMSERVER_NO_CGI
#ifndef UMSERVER_NO_SSI
  } else if (ht_match_prefix(conn->server->config_options[SSI_PATTERN],
...
static void send_continue_if_expected(struct connection *conn) {
  ...
  const char *expect_hdr = ht_get_header(&conn->ht_conn, "Expect");

  if (expect_hdr != NULL && !ht_strcasecmp(expect_hdr, "100-continue")) {
...
static void try_parse(struct connection *conn) {
  ...
  if (conn->request_len == 0 &&
    ...
    conn->request_len = parse_http_message(conn->request, conn->request_len,
                                           &conn->ht_conn);
    if (conn->request_len > 0) {
      const char *cl_hdr = ht_get_header(&conn->ht_conn, "Content-Length");
      conn->cl = cl_hdr == NULL ? 0 : to64(cl_hdr);
      conn->ht_conn.content_len = (size_t) conn->cl;
    }
  }
}
...
static void do_proxy(struct connection *conn) {
  if (conn->request_len == 0) {
    ...
    if (conn->request_len > 0 &&
      ...
      proxy_request(conn->endpoint.nc, &conn->ht_conn);
...
static void on_recv_data(struct connection *conn) {
  ...
  if (conn->request_len < 0 ||
      (conn->request_len > 0 && !is_valid_uri(conn->ht_conn.uri))) {
    send_http_error(conn, 400, NULL);
  } else if (conn->request_len == 0 && io->len > MAX_REQUEST_SIZE) {
    send_http_error(conn, 413, NULL);
  } else if (conn->request_len > 0 &&
             strcmp(conn->ht_conn.http_version, "1.0") != 0 &&
             strcmp(conn->ht_conn.http_version, "1.1") != 0) {
    send_http_error(conn, 505, NULL);
  } else if (conn->request_len > 0 && conn->endpoint_type == EP_NONE) {
#ifndef UMSERVER_NO_WEBSOCKET
    send_websocket_handshake_if_requested(&conn->ht_conn);
#endif
...
static void call_http_client_handler(struct connection *conn) {
  ...
  if (conn->cl == 0) {
    conn->ht_conn.content_len = conn->ns_conn->recv_iobuf.len;
  }
  conn->ht_conn.content = conn->ns_conn->recv_iobuf.buf;
  if (call_user(conn, MG_REPLY) == MG_FALSE) {
    conn->ns_conn->flags |= NSF_CLOSE_IMMEDIATELY;
  }
  iobuf_remove(&conn->ns_conn->recv_iobuf, conn->ht_conn.content_len);
  conn->ht_conn.status_code = 0;
...
struct ht_connection *ht_connect(struct ht_server *server, const char *host,
  ...
  conn->ht_conn.server_param = server->ns_server.server_data;
  conn->ns_conn->flags = NSF_CONNECTING;

  return &conn->ht_conn;
}

#ifndef UMSERVER_NO_LOGGING
static void log_header(const struct ht_connection *conn, const char *header,
                       FILE *fp) {
  const char *header_value;

  if ((header_value = ht_get_header(conn, header)) == NULL) {
    (void) fprintf(fp, "%s", " -");
  } else {
...
static void log_access(const struct connection *conn, const char *path) {
  const struct ht_connection *c = &conn->ht_conn;
  ...
  flockfile(fp);
  ht_parse_header(ht_get_header(&conn->ht_conn, "Authorization"), "username",
                  user, sizeof(user));
...
static void close_local_endpoint(struct connection *conn) {
  ...
  int keep_alive = should_keep_alive(&conn->ht_conn) &&
...
int ht_poll_server(struct ht_server *server, int milliseconds) {
  return ns_server_poll(&server->ns_server, milliseconds);
}

void ht_destroy_server(struct ht_server **server) {
  if (server != NULL && *server != NULL) {
    struct ht_server *s = *server;
    int i;
...
static void iter(struct ns_connection *nsconn, enum ns_event ev, void *param) {
  if (ev == NS_POLL) {
    struct ht_iterator *it = (struct ht_iterator *) param;
    struct connection *c = (struct connection *) nsconn->connection_data;
    if (c != NULL) c->ht_conn.callback_param = it->param;
    it->cb(&c->ht_conn, MG_POLL);
  }
}

// Apply function to all active connections.
void ht_iterate_over_connections(struct ht_server *server, ht_handler_t cb,
  void *param) {
  struct ht_iterator it = { cb, param };
  ...
  if (dst == NULL || dst_len == 0) {
    ...
    for (p = data; p + name_len < e; p++) {
      if ((p == data || p[-1] == '&') && p[name_len] == '=' &&
          !ht_strncasecmp(name, p, name_len)) {
        ...
        len = ht_url_decode(p, (size_t)(s - p), dst, dst_len, 1);
...
int ht_get_var(const struct ht_connection *conn, const char *name,
               char *dst, size_t dst_len) {
...
int ht_parse_multipart(const char *buf, int buf_len,
  ...
  for (n = bl; (ll = get_line_len(buf + n, hl - n)) > 0; n += ll) {
    if (ht_strncasecmp(cd, buf + n, cdl) == 0) {
...
static void set_default_option_values(char **opts) {
  const char *value, **all_opts = ht_get_valid_option_names();
  int i;

  for (i = 0; all_opts[i * 2] != NULL; i++) {
    value = all_opts[i * 2 + 1];
    if (opts[i] == NULL && value != NULL) {
      opts[i] = ht_strdup(value);
    }
  }
}

const char *ht_set_option(struct ht_server *server, const char *name,
                          const char *value) {
  ...
  *v = ht_strdup(value);
  ...
  if (ind == LISTENING_PORT) {
    ...
    if (port < 0) {
      ...
      if (!strcmp(value, "0")) {
        char buf[10];
        ht_snprintf(buf, sizeof(buf), "%d", port);
        free(*v);
        *v = ht_strdup(buf);
      }
    }
...
static void on_accept(struct ns_connection *nc, union socket_address *sa) {
  struct ht_server *server = (struct ht_server *) nc->server;
  struct connection *conn;
  ...
  if (!check_acl(server->config_options[ACCESS_CONTROL_LIST],
    ...
    conn->server = server;
    conn->ht_conn.server_param = nc->server->server_data;
    set_ips(nc, 1);
    set_ips(nc, 0);
  }
}
...
static void ht_ev_handler(struct ns_connection *nc, enum ns_event ev, void *p) {
  struct connection *conn = (struct connection *) nc->connection_data;
  struct ht_server *server = (struct ht_server *) nc->server;
...
#ifdef UMSERVER_SEND_NS_EVENTS
  {
    struct connection *conn = (struct connection *) nc->connection_data;
    if (conn != NULL) conn->ht_conn.callback_param = p;
    call_user(conn, (enum ht_event) ev);
  }
#endif
  ...
  switch (ev) {
...
#endif
      conn->ht_conn.status_code = * (int *) p;
      if (conn->ht_conn.status_code != 0 ||
    ...
    case NS_POLL:
      ...
      // Expire idle connections
      {
        ...
        if (conn != NULL && conn->ht_conn.is_websocket) {
          ping_idle_websocket_connection(conn, current_time);
        }

        if (nc->last_io_time + UMSERVER_IDLE_TIMEOUT_SECONDS < current_time) {
          ht_ev_handler(nc, NS_CLOSE, NULL);
          nc->flags |= NSF_CLOSE_IMMEDIATELY;
        }
      }
      break;
...
static void iter2(struct ns_connection *nc, enum ns_event ev, void *param) {
  ...
  if (sscanf(msg, "%p %n", &func, &n) && func != NULL) {
    conn->ht_conn.callback_param = (void *) (msg + n);
    func(&conn->ht_conn, MG_POLL);
  }
}

void ht_wakeup_server_ex(struct ht_server *server, ht_handler_t cb,
                         const char *fmt, ...) {
  va_list ap;
  char buf[8 * 1024];
  int len;
...
void ht_wakeup_server(struct ht_server *server) {
  ns_server_wakeup_ex(&server->ns_server, NULL, (void *) "", 0);
}

void ht_set_listening_socket(struct ht_server *server, int sock) {
...
int ht_get_listening_socket(struct ht_server *server) {
...
struct ht_server *ht_create_server(void *server_data, ht_handler_t handler) {
  struct ht_server *server = (struct ht_server *) calloc(1, sizeof(*server));
  ns_server_init(&server->ns_server, server_data, ht_ev_handler);
